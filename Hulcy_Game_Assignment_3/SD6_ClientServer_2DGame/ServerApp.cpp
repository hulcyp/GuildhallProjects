...
namespace Monky
{
	...
	void ServerApp::SpawnBulletEvt( NamedProperties& params )
	{
		...
		if( params.get( "pos", pos ) == NamedProperties::SUCCESS )
		{
			if( params.get( "rotation", rotation ) == NamedProperties::SUCCESS )
			{
				...
				Bullet* bullet = new Bullet( vec2f( pos.x, pos.y ), rotation, teamNumber, m_screenWidth, m_screenHeight, true );
	...
	{		
		...
		int packetID = -1;
		ServerPacketType type = SERVER_PACKET_INVALID;
	...
	void ServerApp::SendPlayerUpdate()
	{
		...
		for( int s = 0; s < MAX_NUM_PLAYERS; ++s )
		{
			...
			{
				m_networkPacket->AddData( vec2f( -1.0f, -1.0f ) );	//-1.0f position means the ship is dead
				m_networkPacket->AddData( vec2f( 0.0f, 0.0f ) );
				m_networkPacket->AddData( 0.0f );	//Zero rotation for spaceship
				m_networkPacket->AddData( 0 );	//No ship name sent
			}
			else 
			{
				m_networkPacket->AddData( m_spaceships[s]->getPosition().xy() );
				m_networkPacket->AddData( m_spaceships[s]->getVelocity() );
				m_networkPacket->AddData( m_spaceships[s]->getSGARotation() );
		...
		if( teamNumber < MAX_NUM_PLAYERS && teamNumber >= 0 )
		{
			if( m_players[ teamNumber ] != nullptr )
			{
				if( packetID > m_players[ teamNumber ]->GetMostRecentPacketIDRecieved() )
				{
					...
					m_networkPacket->GetNextChunkOfData( numberOfKeys );
					
					for( int i = 0; i < numberOfKeys; ++i )
					{
						MonkyKey keyCode;
						if( m_networkPacket->GetNextChunkOfData( keyCode ) )
						{
							m_players[ teamNumber ]->onKeyDown( keyCode, 0, 0 );
						}
					}
				}
			}
	...
	std::string& ServerApp::GetShipName( short teamNumber )
	{
		...
		assertion( 0 <= teamNumber && teamNumber < MAX_NUM_PLAYERS, "Team number out of range: %d", teamNumber );
	...
	//----------------------------------------------------------------------
	void ServerApp::clearShipForcesBitFlags()
	{
		for( int i = 0; i < MAX_NUM_PLAYERS; ++i )
		{
			if( m_spaceships[i] != nullptr )
			{
				m_spaceships[i]->clearForcesAppliedBitFlags();
			}
		}
	}
}