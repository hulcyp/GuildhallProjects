...
namespace Monky
{
	...
	bool CommandLine::onKeyDown( MonkyKey keyCode )
	{
		...
		// ctrl + C
		case MONKY_c:
			if( ( InputSystem::getInstance()->getCurrentModifiers() & MONKY_MOD_CTRL ) != 0 )
			{
		...
		case MONKY_LEFT:
			if( m_cursorPos > 0 )
			{
				if( ( ( InputSystem::getInstance()->getCurrentModifiers() & MONKY_MOD_SHIFT ) != 0 ) && m_prevCursPos == -1)
				{
					m_prevCursPos = m_cursorPos;
				}
				else if( ( ( InputSystem::getInstance()->getCurrentModifiers() & MONKY_MOD_SHIFT ) == 0 ) )
				{
					m_prevCursPos = -1;
				}
				--m_cursorPos;
