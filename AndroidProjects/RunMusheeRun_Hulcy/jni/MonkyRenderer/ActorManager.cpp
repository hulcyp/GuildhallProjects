...
namespace Monky
{
	...
	void ActorManager::update( double deltaTime )
	{
		...
		for( auto iter = m_actors.begin(); iter != m_actors.end(); )
		{
			...
			{
				m_actorsToRender.push_back( iter->second );
				++iter;
			}
		}

		std::sort( m_actorsToRender.begin(), m_actorsToRender.end(),
				[]( const Actor* first, const Actor* second ) -> bool
				{
	...
	void ActorManager::renderActors()
	{
		...
		for( int i = 0; i < m_actorsToRender.size(); ++i )
		{
			m_actorsToRender[i]->render();
		}
		m_actorsToRender.clear();
	}
