...
namespace Monky
{
	...
	public:
		...
		bool isPerpendicular( const Vector2<T>& other, T epsilon = MathFuncs<T>::EPSILON ) const;
		...
		Vector2<T> clamp( T floor, T ceil ) const;
		Vector2<T> clamp( const Vector2<T>& floor, const Vector2<T>& ceil ) const;
