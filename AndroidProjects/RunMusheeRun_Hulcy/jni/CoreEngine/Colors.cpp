...
namespace Monky
{
	...
	Color4f Color4f::operator*( float val ) const
	{
		return Color4f( r * val, g * val, b * val, a * val );
	}
	...
	unsigned long Color4f::getColorAsLong() const
	{
		unsigned long color = 0;
		color = color | ( (unsigned long)( r * 255 ) << 16 );
		color = color | ( (unsigned long)( g * 255 ) << 8 );
		color = color | (unsigned long)( b * 255 );
		return color;
	}
	...
	Color4f parseStringToColor4f( const std::string& str, const Color4f& defaultValue )
	{
		...
		if( str[0] == '#' )
		{
			...
			if( res == nullptr )
				MONKY_EXCEPTION( "Invalid hexidecimal value" );
		...
		else
		{
			...
			if( numValues < 3 )
				MONKY_EXCEPTION( "Not enough values to fill a Color4" );
