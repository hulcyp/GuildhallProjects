...
namespace Monky
{
		...
		:	m_stride( 0 )
	{
		...
		{
			DataType type = (DataType)va_arg( args, int );
	...
	unsigned int BufferLayout::numAttributes() const
	{
	...
	int BufferLayout::attributeSize( unsigned int i ) const
	{
		assertion( i < m_layout.size(), "Layout index: %d is out of bounds", i );
		return dataTypeSize( m_layout[i] );
	}

	int BufferLayout::attributeOffset( unsigned int index ) const
	{
	...
	int BufferLayout::numOfDataTypeForAttrib( unsigned int i ) const
	{
		assertion( i < m_layout.size(), "Layout index: %d is out of bounds", i );
