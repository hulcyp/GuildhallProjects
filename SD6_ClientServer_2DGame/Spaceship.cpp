...
namespace Monky
{
	...
	{
		SpaceGameActor::update( deltaTime );		
		SGA_State currentState = m_stateTimeline.Get( 0 );

		if( currentState.spawnBullet && m_weaponTimer->isTimeUp() )
		{
			NamedProperties params;
			params.set( "pos", getPosition() );
			params.set( "rotation", currentState.rot );
			params.set( "teamNumber", m_teamNumber );
			fireEvent( "SpawnBullet", params );
			m_weaponTimer->start();
			//currentState.spawnBullet = false;
	...
	Color4f getTeamColor( short teamNumber )
	{
		...
		if( !isInitialized )
		{
			...
			teamColors[4] = color::GREY;
			teamColors[5] = color::BLACK;
			teamColors[6] = color::DARK_GREEN;
			teamColors[7] = color::WHITE;
		}

		assertion( teamNumber >= 0 && teamNumber < 8, "Team number out of range: %d", teamNumber );
